...
void gap_buffer_print(gap_T g, FILE* output) {
    int i = 0;

    //printf(" start:%*d",3, g->start);
    fprintf(output, " cursor:%*d", 3, g->cursor);
    fprintf(output, " gap_start: %*d", 3, g->gap_start);
    fprintf(output, " gap_end:%*d", 3, g->gap_end);
    fprintf(output, "  [");
...
int gap_buffer_is_empty(gap_T g){
    if(g->cursor == g->gap_start){
        if(g->start == g->gap_start && g->end == g->gap_end){
            return 1;
        }
        return 0;
    }
